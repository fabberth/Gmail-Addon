...
const CuentasXUsuarios = require('./cuentasXusuario.json');
...
app.post(/getAccounts/,async(req, res)=>{
  console.log(req.body.usuarioGoogle);

  var usuarios = CuentasXUsuarios[req.body.usuarioGoogle]
  if(usuarios === undefined) {
    res.send(JSON.stringify({ usuarios:["NN"]}));
  }
  else{
...
app.post(/consulta/,async(req, res)=>{
  ...
  var usuarioAbox = req.body.userAbox;
  console.log(usuarioAbox);
  var estado = false;
  ...
  if (dabaBase == true){
    ...
    const result = await sql.query(`SELECT * FROM GmailAccounts WHERE usuarioGoogle ='${usuarioGoogle}' and usuario = '${usuarioAbox}';`);

    if (result.recordset.length==0){

      var inse = await sql.query(`INSERT INTO GmailAccounts (usuarioGoogle,usuario) VALUES ('${usuarioGoogle}','${usuarioAbox}');`);
      sql.close();

      if(inse.rowsAffected[0]==1){

        console.log('insertado: ',usuarioAbox);
        estado = 'primeraVez';
        funciones.log("INFO",'Primera vez '+usuarioAbox);
...
app.post(/saveDocumentTypeDefault/,async(req, res,)=>{
  ...
  var userAbox = req.body.userAbox;

  var dabaBase = await funciones.conexion();

  if (dabaBase == true){
    const result = await sql.query(`UPDATE GmailAccounts SET documentTypeDefault='${documentTypeDefault}', folderDefault='${folderDefailt != undefined ? folderDefailt : "NULL"}' WHERE usuarioGoogle='${usuarioGmail}' and usuario='${userAbox}';`)
...
app.post(/identificarse/,async(req, res,)=>{
  ...
  var usuarioGmail = req.body.usuarioGmail;
  var usuario = req.body.usuario;
  var datos = await funciones.getDatos(usuarioGmail,usuario);
  var password = req.body.contrasena;
  var token = "";
  var errormsg = null
  var obje = [];
...
function anexo (token,codigo,nArchivo,adjunto,urlSoa){
  ...
  let carga = `<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
                  <Body>
                      <AnexarArchivo xmlns="http://tempuri.org/">
                          <token>${token2}</token>
                          <codigo>${codigo2}</codigo>
                          <nombre>${nArchivo2}</nombre>
                          <descripcion></descripcion>
                          <bytearray_archivo>${adjunto2}</bytearray_archivo>
                      </AnexarArchivo>
                  </Body>
                </Envelope>`;
  

  axios({ method: 'post', url: urlSoa, data: carga, maxContentLength: Infinity, maxBodyLength: Infinity,
            headers: 
            {  'Content-Type':'text/xml;charset=utf-8',
              ...
              console.log("Anexo cargado al documento: " + codigo);
              return true
            })
            .catch(function (error) {
              //console.log(error)
              console.log('error al cargar el anexo muy grande ' + 'codigo doc: '+codigo2);
              //console.log(error);
              return false
            });
...
app.post(/crearDocumento/,async(req, res)=>{
  ...
  var userAbox = req.body.userAbox;
  console.log(userAbox+"-"+usuarioGmail);
  ...
  if(req.body.folder === undefined || req.body.folder == "null"|| req.body.folder == "")
  {
    folder = "";
  }
  else
  ...
  var datos = await funciones.getDatos(usuarioGmail,userAbox);
  ...
  var metadataValue = `<![CDATA[<?xml version="1.0" encoding="UTF-8"?>
							  ...
							  <metadataValue code="Sop.Defecto.NumFolios">1</metadataValue>
  ...
  try
  {
    ...
    if (codigoDocument.length > 4)
    {
      var urlAbo = datos.urlAbox;
        ...
        funciones.log("INFO","Documento creado: "+codigoDocument+" "+usuarioGmail + "-"+ userAbox);
        console.log("Documento creado: "+codigoDocument+" "+usuarioGmail + "-"+ userAbox);
...
//Cierra la sesion borrando el token 
 app.post(/cerrarSesion/,async(req, res)=>{
    ...
    var userAbox = req.body.userAbox;
    var msg = await funciones.borrar(usuarioGoogle,userAbox);
...
app.get('/pagina/:file?', function (req, res)
{
  ...
  index = index.replace('%%usuarioAbox%%',req.query.usuario)
	res.send(index);
});

app.get('/paginaIdenti/', async function (req, res)
{
  var datos = await funciones.getDatos(req.query.correo,req.query.usuario)
...
{
  https.createServer({
    cert: fs.readFileSync('src/certificados/cert_adptg.crt'),
    key: fs.readFileSync('src/certificados/llave_lista.key')
  },app).listen(app.get('port'), () => {
