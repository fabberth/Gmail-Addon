...
let guardar = async function guardar (usuarioGoogle,token,urlAbox,urlSoap,usuario,documentTypeCode, folder)
{
  
    var dabaBase = await conexion();
    if (dabaBase == true)
    {
        const result = await sql.query(`UPDATE GmailAccounts SET token='${token}',urlAbox='${urlAbox}',urlSoap='${urlSoap}',documentTypeCode='${documentTypeCode}', folders='${folder}' WHERE usuarioGoogle='${usuarioGoogle}' and usuario='${usuario}';`)
...
let getDatos = async function getDatos(usuarioGoogle, userAbox)
{
    ...
    if (dabaBase == true)
    {
      ...
      const result = await sql.query(`SELECT * FROM GmailAccounts WHERE usuarioGoogle ='${usuarioGoogle}' and usuario='${userAbox}';`)
...
let borrar = async function borrar (usuarioGoogle,userAbox)
{
    var dabaBase = await conexion();
    if (dabaBase == true)
    {
        const result = await sql.query(`UPDATE GmailAccounts SET token=NULL WHERE usuarioGoogle='${usuarioGoogle}' and usuario = '${userAbox}';`)
        //console.log(result.rowsAffected)
...
let GuardarToken = async function GuardarToken(usuarioGoogle,token,usuario)
{
    ...
    if (dabaBase == true)
    {
      ...
      const result = await sql.query(`UPDATE GmailAccounts SET token='${token}',usuario='${usuario}' WHERE usuarioGoogle='${usuarioGoogle}' and usuario = '${usuario}';`)
      //console.log(result.rowsAffected)
 ...
 */
 let findAllFolderByUser = function findAllFolderByUser(urlSoap,token)
 {
     var xml = `<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tem="http://tempuri.org/">
                     <soapenv:Header/>
                     <soapenv:Body>
 ...
 */
let createDocument = function createDocument(urlSoap,token,title,documentTypeCode,description,filename,fileBytearray,folder,metadata="")
{
    let xml = `
