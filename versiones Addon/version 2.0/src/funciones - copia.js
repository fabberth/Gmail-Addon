...
let guardar = async function guardar (usuarioGoogle,token,urlAbox,urlSoap,usuario,documentTypeCode, folder)
{
    ...
    if (dabaBase == true)
    {
        const result = await sql.query(`UPDATE GmailAccounts SET token='${token}',urlAbox='${urlAbox}',urlSoap='${urlSoap}',usuario='${usuario}',documentTypeCode='${documentTypeCode}', folders='${folder}' WHERE usuarioGoogle='${usuarioGoogle}';`)
...
let getDatos = async function getDatos(usuarioGoogle,nCedula)
{
    ...
    if (dabaBase == true)
    {
      ...
      const result = await sql.query(`SELECT * FROM GmailAccounts WHERE usuarioGoogle ='${usuarioGoogle}' and nCedula ='${nCedula}' ;`)
        ...
        try
        {
            datos = {
                ...
                usuario:result.recordset[0].nCedula,
    ...
    <Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
        <Body>
            <CreateDocument xmlns="http://tempuri.org/">
                ...
                <folderCode>${folder === undefined ? folder : ""}</folderCode>
