  ...
  if (dabaBase == true)
  {
    ...
    else
    {
      ...
      if (result.recordset[0].token != null && result.recordset[0].documentTypeCode != null)
      {
        ...
        sql.close()
        //console.log(estado);
        res.send(JSON.stringify({ tipoDocumental: result.recordset[0].documentTypeCode, estado:estado, tipoDefault:result.recordset[0].documentTypeDefault}))
            ...
            var dataJsonDocument = JSON.parse(parserXml.toJson(dataXmlDocument.data));
              ...
              if(dataJsonDocument['s:Envelope']['s:Body'].FindAllEntityTypesByUserResponse.FindAllEntityTypesByUserResult['a:exitvalue'] != 'does not have a documentary type assigned')
              {
                ...
                if (entityTypeByUser.length)
                {
                    ...
                    var guarda = await funciones.guardar(usuarioGmail,token,urlAbox,urlSoap,usuario,JSON.stringify(obje));
                    if(guarda == true)
                    {
                      res.send(JSON.stringify({"error":"","message":obje}));
                    }
                    else
                    {
...
app.post(/saveDocumentTypeDefault/,async(req, res,)=>{
  ...
  if (dabaBase == true){
    const result = await sql.query(`UPDATE GmailAccounts SET documentTypeDefault='${documentTypeDefault}' WHERE usuarioGoogle='${usuarioGmail}';`)
          ...
          var dataJsonDocument = JSON.parse(parserXml.toJson(dataXmlDocument.data));
            ...
            if(dataJsonDocument['s:Envelope']['s:Body'].FindAllEntityTypesByUserResponse.FindAllEntityTypesByUserResult['a:exitvalue'] != 'does not have a documentary type assigned')
            {
              ...
              if (entityTypeByUser.length)
              {
                  ...
                  var guarda = await funciones.guardar(usuarioGmail,token,datos.urlAbox,datos.urlSoap,usuario,JSON.stringify(obje));
                  if(guarda == true)
                  {
                    res.send(JSON.stringify({"error":"","message":obje}));
                  }
...
app.post(/crearDocumento/,async(req, res)=>{
  ...
  var entityType = req.body.document;
  ...
  try
  {
    var createDocument = await funciones.createDocument(datos.urlSoap, datos.token, asunto, entityType, bodyCorreo, idMessage, eml, metadataValue);
